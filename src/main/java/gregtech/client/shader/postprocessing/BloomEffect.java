...
public class BloomEffect {
    ...
    private static void cleanUP(int lastWidth, int lastHeight) {
        if (downSampleFBO == null || downSampleFBO.length != LumenizedConfig.nMips) {
            ...
            downSampleFBO = new Framebuffer[LumenizedConfig.nMips];
            upSampleFBO = new Framebuffer[LumenizedConfig.nMips];

            int resX = lastWidth / 2;
            int resY = lastHeight / 2;

            for (int i = 0; i < LumenizedConfig.nMips; i++) {
